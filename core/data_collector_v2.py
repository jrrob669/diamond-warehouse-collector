...
VERSION MISE À JOUR avec Term Structure (RR25 par DTE)
"""
import pandas as pd
import logging
...
class OptionsDataCollector:
    ...
    def calculate_daily_metrics(self, symbol: str, date: str) -> tuple:
        ...
        # 4. Skew Metrics (Global - toutes expirations mélangées)
        skew_metrics = calculate_skew_metrics(df_greeks)
        metrics.update(skew_metrics)
        
        # 5. ⭐ NOUVEAU : Term Structure Metrics (RR25 par DTE)
        logger.info("Calculating term structure metrics (RR25 by DTE)...")
        term_metrics = calculate_term_structure_metrics(df_greeks)
        metrics.update(term_metrics)
        
        # Log term structure
        if 'rr25_0dte' in term_metrics:
            logger.info(f"RR25 0DTE: {term_metrics.get('rr25_0dte', 'N/A')}")
            logger.info(f"RR25 7DTE: {term_metrics.get('rr25_7dte', 'N/A')}")
            logger.info(f"RR25 30DTE: {term_metrics.get('rr25_30dte', 'N/A')}")
            logger.info(f"RR25 60DTE: {term_metrics.get('rr25_60dte', 'N/A')}")
